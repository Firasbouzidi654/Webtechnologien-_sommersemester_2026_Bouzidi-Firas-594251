...
describe('medication timeline API mapping', () => {
  it('loads the saved medication time and status after refresh', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((url) => {
      const data = String(url).endsWith('/api/children')
        ...
        : [{ id: 4, name: 'Salbutamol', childId: 1, childName: 'Emma', dosage: '1 puff', time: '08:30', scheduledDate: new Date().toISOString().slice(0, 10), status: 'TAKEN' }];
  ...
  it('sends the selected date when creating a medication', async () => {
    localStorage.setItem('kindercare-simple-user', JSON.stringify({ id: 12, role: 'PARENT' }));
    const fetchMock = vi.fn((url, options = {}) => {
      ...
      return Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ id: 9, name: 'Vitamin D', childId: 1, childName: 'Emma', scheduledDate: '2026-06-23' })
      });
    });
    ...
    expect(payload).toMatchObject({ scheduledDate: '2026-06-23' });
